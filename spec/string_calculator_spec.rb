...
RSpec.describe StringCalculator do
  describe '#add' do
    ...
    it 'throws an exception for a single negative number' do
      calculator = StringCalculator.new
      expect { calculator.add('-1') }.to raise_error(RuntimeError, 'RuntimeError: negative numbers not allowed: -1')
    end

    it 'throws an exception for multiple negative numbers' do
      calculator = StringCalculator.new
      expect { calculator.add('2,-4,-5,6') }.to raise_error(RuntimeError, 'RuntimeError: negative numbers not allowed: -4,-5')
    end
    ...
    it 'allows multiple single-character delimiters' do
      calculator = StringCalculator.new
      expect(calculator.add("//[*][%]\n1*2%3")).to eq(6)
    end
    it 'allows multiple multi-character delimiters' do
      calculator = StringCalculator.new
      expect(calculator.add("//[**][%%]\n1**2%%3")).to eq(6)
    end
    it 'allows multiple delimiters with different lengths' do
      calculator = StringCalculator.new
      expect(calculator.add("//[*][%%]\n1*2%%3")).to eq(6)
    end
